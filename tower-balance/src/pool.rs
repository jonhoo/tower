...
//!
//! The pool uses `poll_ready` as a signal indicating whehter additional services should be spawned
...
/// that lets you configure how a [`Pool`] determines whether the underlying service is loaded or
/// not. See the [module-level documentation](index.html) and the builder's methods for details.
...
impl Builder {
    ...
    {
        let s = make_service.make_service(target.clone());
